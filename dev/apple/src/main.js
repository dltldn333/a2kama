...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const dockButtons = Array.from(document.querySelectorAll("[data-page-target]"));
    ...
    if (!root || !scrollViewport || !scrollContent || !lockScreen)
        return;
    ...
    const getViewportHeight = () => scrollViewport.clientHeight || root.clientHeight || window.innerHeight;
    const setViewportHeight = () => document.documentElement.style.setProperty("--app-vh", `${getViewportHeight()}px`);
    setViewportHeight();
    const getPagePoints = () => {
    ...
    const previewDockButton = (button) => {
        if (dragTargetButton === button)
            return;
        dragTargetButton = button;
        dockButtons.forEach((item) => item.classList.toggle("is-drag-target", item === button));
    };
    const stepSpring = (value, velocity, target, stiffness, damping, deltaSeconds) => {
    ...
    const updateDockPhysics = (_time, deltaTime) => {
        if (!isDockDragging || !dockHighlight)
            return;
        ...
        if (idleFor > 24) {
            ...
            if (Math.abs(dragVelocityX) < 0.005)
                dragVelocityX = 0;
        }
        ...
        const rightDamping = isMoving
            ? rightIsLeading
                ...
                : trailingDamping
            : restingDamping;
        [physicsLeft, physicsLeftVelocity] = stepSpring(physicsLeft, physicsLeftVelocity, targetLeft, leftStiffness, leftDamping, deltaSeconds);
        [physicsRight, physicsRightVelocity] = stepSpring(physicsRight, physicsRightVelocity, targetRight, rightStiffness, rightDamping, deltaSeconds);
        ...
        if (renderedWidth > maxWidth) {
            ...
            if (dragDirection > 0)
                physicsLeft = physicsRight - renderedWidth;
            else
                physicsRight = physicsLeft + renderedWidth;
        }
        ...
        const areaPreservingScaleY = (dragWidth * dragBaseScaleY) / Math.max(renderedWidth, 1);
    ...
    const moveDockHighlight = (button, animate = true) => {
        ...
        const currentWidth = Number(gsap.getProperty(dockHighlight, "width")) || dockHighlight.offsetWidth;
        const currentScaleY = Number(gsap.getProperty(dockHighlight, "scaleY")) || 1;
        const isAlreadyStretched = Math.abs(currentWidth - targetWidth) > 2 || Math.abs(currentScaleY - 1) > 0.04;
        ...
        const edgeLag = clamp(Math.abs(targetX - currentX) * 0.34, 10, targetWidth * 0.65);
    ...
    const activateDockButton = (pageId, animate = true, forceMove = false) => {
        ...
        if (displayId === activeDockId && !forceMove)
            return;
        const activeButton = dockButtons.find((button) => button.dataset.pageTarget === displayId);
        if (activeButton) {
            moveDockHighlight(activeButton, animate && (activeDockId !== "" || forceMove));
        }
    ...
    const setRangeValue = (control, nextValue) => {
        ...
        if (control.dataset.range === "glass-opacity") {
            ...
            if (opacityOutput)
                opacityOutput.textContent = `${roundedValue}%`;
        }
    };
    const rangeHandlers = rangeControls.map((control) => {
        ...
        const setFromPointer = (event) => {
            ...
            const min = Number(control.dataset.min ?? 0);
            const max = Number(control.dataset.max ?? 100);
            const progress = clamp((event.clientX - rect.left) / Math.max(rect.width, 1));
        ...
        setRangeValue(control, Number(control.dataset.value ?? control.dataset.min ?? 0));
    ...
    const updateDock = (scroll) => {
        ...
        const opacity = scroll >= viewportHeight
            ? 1
    ...
    const updateLockScreen = (scroll) => {
        ...
        const forwardGlass = clamp((progress - 0.82) / 0.08) *
            (1 - clamp((progress - 0.97) / 0.03));
        const reverseGlass = clamp((progress - 0.06) / 0.08) *
    ...
    const goToPage = (targetIndex, duration = 0.82, onSettled) => {
        ...
        if (!target)
            return;
        if (settledPageIndex === 0 && boundedIndex > 0)
            transitionMode = "forward";
    ...
    const settleToNearestPage = () => {
        ...
        if (scroll > origin.top + 1 && settledPageIndex < points.length - 1) {
            ...
            if (progress >= 0.5)
                targetIndex = settledPageIndex + 1;
        }
    ...
    const handleResize = () => {
        ...
        const activeButton = dockButtons.find((button) => button.classList.contains("is-active"));
        if (activeButton)
            moveDockHighlight(activeButton, false);
    };
    const handleUnlock = () => {
        transitionMode = "forward";
        goToPage(1, 1.05);
    ...
    const handleDockPointerDown = (event) => {
        ...
        const hitPadding = 8;
        const startedOnHighlight = event.clientX >= highlightRect.left - hitPadding &&
    ...
    return () => {
        ...
        gsap.ticker.remove(updateDockPhysics);
        dockHandlers.forEach(({ button, handler }) => button.removeEventListener("click", handler));
        rangeHandlers.forEach(({ control, thumb, pointerDown, pointerMove, pointerUp, keyDown }) => {
